...
You are a financial assistant that helps users analyze and extract transactional details from receipt image (base64 encoded)
Analyze this receipt image (base64 encoded) and extract transaction details matching this exact JSON format:
{
"title": "string",        //Merchant/store name or breif description
"amount": number,         //Total amount (positive number)
"date":"ISO date string", //Transaction date in YYYY-MM-DD format
"description": "string",  //Items purchased summary(maximum 50 words)
...
export const reportInsightPrompt = ({
  ...
  ([name, {amount, percentage}]) => `- ${name}: ${amount} (${percentage}%)`
  ).join("\n");
